...
def predict():
   ...
   # comment=comment[0]
    ...
    if len(s)==0:
        return render_template('index.html', pred='Add some comment!!',texta=s
                              )
   ...
   # prediction = model.predict_proba(comment)
    #print(prediction)
    if output > str(0.4):
        return render_template('index.html', pred='Bullying Comment!!!\nProbability of being bully comment is {}. Report it!'.format(output),texta=s
                              )
    else:
        return render_template('index.html', pred='Not a Bullying Comment.\n Probability of being bully comment is {}'.format(output),texta=s
                               )
